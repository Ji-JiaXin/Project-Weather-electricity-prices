...
class DownloadAPI(object):
    ...
    def download_chart_data_zip()
        Makes an API request, checks whether it was succesfull using the status code. In case of success it downloads the data
        into zip file.
        Parameters:
            Same as previous method chart_data_filter_region_filter_copy_region_copy_resolution_timestamp_json_get(). 
    
    def process_zip_file()
        It unzips the created file from previous method download_chart_data_zip() and save it as Pandas Dataframe.
        Parameters:
            zip_file_path(str)  
    ...
    def download_chart_data_zip(self, filter, filter_copy, region_copy, timestamp, region="DE", resolution="day", **kwargs):
        """
        Download the data as a zip file. It calls the previous method and saves it into zip file.
        Returns:
            If the data was succesfully downloaded a zip file is created, otherwise None. 
        """
        try:
            # API request
            ...
            # Checking the status code - if the request was successful = status code 200
            if response.status == 200:
                # Creating the zip file in case of successful request
                with ZipFile(f"chart_data_{filter}_{region}_{filter_copy}_{region_copy}_{resolution}_{timestamp}.zip", 'w') as zip_file:
                    # Add the response to the created zip file
                    zip_file.writestr('downloaded_data.json', response.data.to_str())
            else:
                print(f"Error: {response.status}")
        except Exception as e:
            print("Something went wrong while downloading the data, try to check your previous steps.")
            print(e)
            pass
    ...
    def process_zip_file(self, zip_file_path):
        ...
        try:
            ...
            with ZipFile(zip_file_path, 'r') as zip_file:
                # Read the data 
                json_data = zip_file.read('downloaded_data.json')
                # Convert json data to Pandas DataFrame
                df = pd.read_json(json_data)
                print(df)
        except Exception as e:
            print("Error occured while extracting the zip file, please check previous steps or the file might have been deleted or renamed.")
            print(e)
    
download_api = DownloadAPI()
df = download_api.download_chart_data_zip(filter=1223, filter_copy=1223, region_copy="DE", timestamp=1234567890)
if df is not None:
